...
Function Invoke-BootStrapCommerceServicesTask {
    ...
    Invoke-RestMethod $UrlCommerceShopsServicesBootstrap -TimeoutSec 12000 -Method PUT -Headers $headers
...
Function Invoke-InitializeCommerceServicesTask {
    ...
    foreach ($env in $Environments) {
        ...
        $result = Invoke-RestMethod $initializeUrl -TimeoutSec 12000 -Method Get -Headers $headers -ContentType "application/json"
        ...
        do {
            ...
            $result = Invoke-RestMethod $checkUrl -TimeoutSec 12000 -Method Get -Headers $headers -ContentType "application/json"
			Write-Host "aaaaaa" + $result.Messages[0]
            if ($result.ResponseCode -ne "Ok" -and $result.Messages[0].CommerceTermKey -eq "BookNameAlreadyInUse") {
				break
...
Function Invoke-EnsureSyncDefaultContentPathsTask {
    ...
    foreach ($env in $Environments) {
        ...
        Write-Host $ensureUrl
		$result = Invoke-RestMethod $ensureUrl -TimeoutSec 12000 -Method PUT -Headers $headers  -ContentType "application/json" 
		Write-Host $result
        $checkUrl = $UrlCheckCommandStatus -replace "taskIdValue", $result.TaskId
		Write-Host $checkUrl
        $sw = [system.diagnostics.stopwatch]::StartNew()
        ...
        do {
            ...
            Write-Host "Checking if $($checkUrl) has completed ..." -ForegroundColor White
            $result = Invoke-RestMethod $checkUrl -TimeoutSec 12000 -Method Get -Headers $headers -ContentType "application/json"
			Write-Host "AA" + $result.Messages
			Write-Host "aa" + $result.Messages[0].CommerceTermKey
			Write-Host "bb" + $result.Messages[1]
			
            if ($result.ResponseCode -ne "Ok" -and $result.Messages[0].CommerceTermKey -eq "ContentPathAlreadyExists") {
                break
				$(throw Write-Host "Ensure/Syncs default content paths for environment $($env) failed, please check Engine service logs for more info." -Foregroundcolor Red)
            }
