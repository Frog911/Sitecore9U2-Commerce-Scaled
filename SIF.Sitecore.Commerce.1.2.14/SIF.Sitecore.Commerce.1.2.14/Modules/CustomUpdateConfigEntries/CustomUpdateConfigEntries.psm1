Function Invoke-UpdateBizFXConfigTask {
  PARAM
  (
    [String]$BizFXDirPath=$(throw 'Parameter -InstallDirPath is missing!'),
	[String]$EngineUri=$(throw 'Parameter -InstallDirPath is missing!'),
	[String]$IdentityServerUri=$(throw 'Parameter -InstallDirPath is missing!'),
	[String]$BizFxUri=$(throw 'Parameter -InstallDirPath is missing!')
  )
   
				# Custom UpdateBizFXConfig the BizFx content
				Write-Host "Custom UpdateBizFXConfig the BizFx content" -ForegroundColor Yellow ;
				
				$pathToJson  = $(Join-Path -Path $BizFXDirPath -ChildPath "assets\config.json")
				Write-Host $pathToJson
				$originalJson = Get-Content $pathToJson -Raw  | ConvertFrom-Json
				
				$originalJson.EngineUri =  $EngineUri
				$originalJson.IdentityServerUri = $IdentityServerUri	
				$originalJson.BizFxUri = $BizFxUri
...
Function Invoke-UpdateIdentityconfigTask {
  PARAM
  (
    [String]$IdentityDirPath=$(throw 'Parameter -InstallDirPath is missing!'),
	[String]$BizFxUri=$(throw 'Parameter -InstallDirPath is missing!'),
	[String]$Thumbprint=$(throw 'Parameter -InstallDirPath is missing!'),
	[String]$SQLServerUserName=$(throw 'Parameter -InstallDirPath is missing!'),
	[String]$SQLServerPassword=$(throw 'Parameter -InstallDirPath is missing!')	
  )    
	$pathToJson  = $(Join-Path -Path $IdentityDirPath -ChildPath "wwwroot\appsettings.json")
	$BizFzUriWithoutProtocol =$BizFxUri -replace "https://",""
	$originalJson = Get-Content $pathToJson -Raw  | ConvertFrom-Json
	$originalJson.AppSettings.IDServerCertificateThumbprint = $Thumbprint
	$originalJson | ConvertTo-Json -Depth 100 -Compress | set-content $pathToJson
	Write-Host "AMIT"
	Write-Host $originalJson.AppSettings.SitecoreMembershipOptions.ConnectionString
	Write-Host "AMIT"
	(Get-Content -Path $pathToJson).Replace('Integrated Security=True', 'User ID=' + $SQLServerUserName + ';Password=' + $SQLServerPassword + ';Integrated Security=False') | Set-Content $pathToJson
...
Register-SitecoreInstallExtension -Command Invoke-UpdateBizFXConfigTask -As UpdateBizFXConfig -Type Task -Force
Register-SitecoreInstallExtension -Command Invoke-UpdateIdentityconfigTask -As UpdateIdentityconfig -Type Task -Force
